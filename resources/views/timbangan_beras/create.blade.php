    ...
    <div class="px-3 py-4 sm:px-4 lg:px-5">
        <div class="grid grid-cols-1 gap-4 items-start xl:grid-cols-[minmax(0,520px)_minmax(0,1fr)]">
            <div class="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm max-w-full">
                ...
                <form method="POST" action="{{ route('timbangan-beras.store') }}" class="px-4 py-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="flex overflow-hidden rounded-sm border border-slate-300 {{ $isReadOnly ? 'bg-slate-100' : 'bg-white focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500' }}">
                                <input name="kadar_broken" type="number" min="0" step="0.01" value="{{ old('kadar_broken') }}" {{ $isReadOnly ? 'disabled' : '' }} class="w-full border-0 px-4 py-2.5 text-slate-700 focus:outline-none focus:ring-0 {{ $isReadOnly ? 'bg-slate-100' : '' }}" placeholder="">
                                <span class="inline-flex items-center border-l border-slate-300 bg-slate-50 px-3 text-sm italic text-slate-500">%</span>
                            </div>
                            @error('kadar_broken')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
                ...
                <div class="overflow-x-auto px-4 py-4">
                        ...
                        <tbody>
                            @forelse ($rows as $row)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="border border-slate-200 px-3 py-2 text-center">{{ $row->kadar_broken !== null && $row->kadar_broken !== '' ? $row->kadar_broken . '%' : '-' }}</td>
                                    <td class="border border-slate-200 px-3 py-2 text-center">
                                        @if (!$isReadOnly)
                                            ...
                                            <div class="flex items-center justify-center gap-2 text-gray-500">
                                                ...
                                                <form method="POST" action="{{ route('timbangan-beras.destroy', $row->id) }}" onsubmit="return confirm('Yakin hapus penimbangan ke-{{ $row->timbang_ke }}?');" class="inline-flex m-0 p-0">
                                                        @csrf
                                                        @method('DELETE')
                                            ...
                                            @include('timbangan_beras.edit', ['timbangan' => $row])
                                        @else
                    ...
                    @if ($isProcessing)
                        @if (!$isReadOnly)
                            <form method="POST" action="{{ route('timbangan-beras.selesai_timbang', $terimaBb->id) }}">
                                @csrf
                                <div class="mt-4">
                        ...
                        @else
                            <div class="mt-4">
                                <label class="mb-2 block text-sm font-semibold text-slate-800">Keterangan</label>
                                <textarea name="keterangan" rows="3" disabled class="w-full rounded-sm border border-slate-300 bg-slate-100 px-4 py-2.5 text-slate-700 focus:border-blue-500 focus:ring-blue-500">{{ $terimaBb->keterangan }}</textarea>
                            </div>
                        @endif
                    @elseif ($isFinished)
                        <div class="mt-4">
