<x-app-layout>
        ...
        <div class="grid grid-cols-1 gap-4 sm:gap-5 xl:grid-cols-2 xl:gap-6 items-start">
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl xl:sticky xl:top-4">
                ...
                <div class="max-h-[60vh] sm:max-h-[calc(100vh-220px)] overflow-y-auto overflow-x-auto" id="rincian-tabel-container">
                    @include('pembagian_gabah._rincian_tabel', [
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                ...
                <form id="pembagian-form" class="pembagian-form-body p-5">
                    ...
                    <fieldset class="space-y-6" {{ $isApproved ? 'disabled' : '' }}>
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="text" value="{{ $terimaBg->nopol ?? '-' }}" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Nama Supplier</label>
                                <input type="text" value="{{ $terimaBg->nama_supplier ?? '-' }}" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Jenis Bahan</label>
                                <input type="text" value="{{ $terimaBg->jenis_bahan ?? '-' }}" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Kemasan Pakai</label>
                                <input type="text" value="-" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Tempat Simpan</label>
                                <input type="text" value="{{ $terimaBg->tempat_simpan ?? '-' }}" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                            <div class="md:col-span-2">
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Penggunaan Palet</label>
                                <input type="text" value="{{ $terimaBg->penggunaan_palet ?? '-' }}" disabled class="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="aroma_gabah" id="aroma_gabah" required class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-700 focus:border-emerald-500 focus:ring-emerald-500 transition-colors">
                                    <option value="">Pilih Aroma</option>
                                    @foreach ($aromaOptions as $option)
                                        <option value="{{ $option->id }}" {{ old('aroma_gabah') == $option->id ? 'selected' : '' }}>{{ $option->kriteria }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-semibold text-slate-700">Sorter Gabah</label>
                                <select name="sorter_gabah" id="sorter_gabah" required class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-700 focus:border-emerald-500 focus:ring-emerald-500 transition-colors">
                                    <option value="">Pilih Sorter Gabah</option>
                                    <option value="Ya" {{ old('sorter_gabah') == 'Ya' ? 'selected' : '' }}>Ya</option>
                                    <option value="Tidak" {{ old('sorter_gabah') == 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input name="pembagian_ke" id="pembagian_ke" type="text" value="{{ old('pembagian_ke', $nextSorting ?? '') }}" required class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-700 focus:border-emerald-500 focus:ring-emerald-500 transition-colors" placeholder="Nomor pembagian gabah" />
                            </div>
                            <div>
                    ...
                    <div class="pembagian-actions flex items-center gap-3 pt-4 border-t border-slate-200">
                        @if ($isApproved)
                            <div class="flex w-full items-center justify-center gap-2 text-emerald-700 font-semibold bg-emerald-50 px-4 py-3 rounded-lg border border-emerald-200">
                                ...
                                @if (strtolower(trim($terimaBg->status ?? '')) === 'checked')
                                    Pembagian Gabah Sudah Di-check (Menunggu Approval)
                                @else
                                    Pembagian Gabah Sudah Disetujui (Approved)
                                @endif
                            </div>
                        @else
                            <div class="flex w-full items-center justify-between gap-3 flex-wrap">
                                ...
                                @php
                                    $hasTimbanganCheck = \App\Models\ReportTimbangGabah::where('id_bahan', (string) $terimaBg->id)->exists();
                                    $hasUnsortedCheck = \App\Models\ReportTimbangGabah::where('id_bahan', (string) $terimaBg->id)->where('sorting', 0)->exists();
                                    $allPembagianFilledCheck = $hasTimbanganCheck && !$hasUnsortedCheck;
                                @endphp

                                @if ($allPembagianFilledCheck && strtolower(trim($terimaBg->status ?? '')) === 'finish')
                                    <button type="submit" form="check-form" id="check-btn" class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-3 text-sm font-semibold text-white shadow-md transition hover:from-blue-700 hover:to-indigo-800 active:scale-95">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                ...
                @if (!$isApproved && strtolower(trim($terimaBg->status ?? '')) === 'finish')
                    <form id="check-form" method="POST" action="{{ route('pembagian_gabah.check', $terimaBg->id) }}" class="hidden">
                        @csrf
                    </form>
                @endif
            </div>
    ...
    <script>
        ...
        async function refreshTable() {
            try {
                const response = await fetch(`/pembagian-gabah/${terimaBbId}/rincian`, {
                    headers: {
                ...
                if (data.success) {
                    ...
                    Object.keys(data.data.grouped).sort((a, b) => a - b).forEach(sorting => {
                        ...
                        const isEditableGroup = !isApproved && activeEditingSortingNumber && String(activeEditingSortingNumber) === String(sorting);
                        
                        html += `
                            <table class="min-w-[820px] w-full text-sm mb-4 sorting-group-table" data-sorting-number="${sorting}">
                                <thead>
                                    <tr class="sorting-group-header ${isApproved ? 'cursor-default' : 'cursor-pointer hover:bg-emerald-100 transition-colors'}" ${isApproved ? '' : `data-sorting-number="${sorting}"`}>
                                        <th colspan="7" class="border-b border-slate-200 px-4 py-3 text-left text-emerald-800">
                                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                <span class="flex flex-wrap items-center gap-2">
                                                    ...
                                                    Sorting Ke-${sorting} ${isApproved ? '' : '<span class="text-xs text-emerald-600">(klik untuk edit)</span>'}
                                                </span>
        ...
        async function loadPembagianDetail(sortingNumber) {
            try {
                const response = await fetch(`/pembagian-gabah/${terimaBbId}/detail/${sortingNumber}`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                });
                ...
                if (data.success) {
                    ...
                    document.getElementById('sorter_gabah').value = pembagian.sorter == 1 ? 'Ya' : 'Tidak';
                    document.getElementById('warna').value = pembagian.warna || '';
                    document.getElementById('aroma_gabah').value = pembagian.aroma || '';
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', async function (e) {
                ...
                try {
                    ...
                    const response = await fetch('{{ route('pembagian_gabah.store') }}', {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                    ...
                    if (data.success) {
                        ...
                        // If all rows are sorted (no checkboxes left in the unsorted section), reload to show Checked button
                        const remainingUnsorted = document.querySelectorAll('input[name="selected_rows[]"]').length;
                        if (remainingUnsorted === 0) {
                            setTimeout(() => {
