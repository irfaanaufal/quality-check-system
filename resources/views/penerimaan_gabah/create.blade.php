<x-modal name="penerimaan-gabah-create-modal" maxWidth="4xl" focusable>
	<div
		x-data="{
			setDefaults() {
				const now = new Date();
		...
		x-init="setDefaults()"
		x-on:open-modal.window="if ($event.detail === 'penerimaan-gabah-create-modal') setDefaults()"
		class="bg-gray-100"
	>
		<div class="bg-gray-100 px-5 py-5 sm:px-6">
			<div class="rounded-md border border-gray-300 bg-white shadow-sm">
				<div class="border-b border-gray-200 px-4 py-3 text-lg font-normal text-gray-700">Form Penerimaan Gabah</div>

				<form method="POST" action="{{ route('penerimaan_gabah.store') }}" class="px-4 py-4 sm:px-5">
					@csrf
					<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
						...
						<div class="space-y-5">
							...
							<div x-data="searchableSelect(@js(collect($varietasGabah ?? [])->map(fn ($varietas) => ['value' => $varietas->id, 'label' => $varietas->alias])->values()), 'Pilih Jenis Gabah')" class="relative">
								<label for="jenis_gabah" class="mb-1 block text-base font-semibold text-gray-800">Jenis Gabah</label>
								<input type="text" id="jenis_gabah" class="w-full rounded-sm border border-gray-300 bg-white px-4 py-3 pr-10 text-lg text-gray-800 focus:border-blue-500 focus:ring-blue-500" :placeholder="placeholder" :class="open ? 'border-blue-500 ring-1 ring-blue-500' : ''" x-model="query" x-on:focus="open = true" x-on:click="open = true" x-on:input="open = true; value = ''" x-on:keydown.enter.prevent="resolveTypedValue()" x-on:blur="setTimeout(() => { open = false; resolveTypedValue(); }, 150)">
					...
					<div class="mt-6 border-t border-gray-200 pt-5">
						<div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
							...
							<button type="button" x-on:click.prevent="$dispatch('close-modal', 'penerimaan-gabah-create-modal')" class="inline-flex w-full items-center justify-center rounded-md bg-red-600 px-5 py-2.5 text-base font-medium text-white shadow-sm transition hover:bg-red-700 sm:w-auto">Cancel</button>
						</div>
					</div>
				</form>
			</div>
